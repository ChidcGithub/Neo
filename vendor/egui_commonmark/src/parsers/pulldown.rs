...
// Neo compatibility additions; upstream parser and event renderer remain intact.
fn safe_link(url: &str) -> bool {
    let lower = url.to_ascii_lowercase();
    !url.chars().any(char::is_control)
        && (lower.starts_with("https://") || lower.starts_with("http://") || url.starts_with('#'))
}
...
fn table_text_job(
    ...
    for (event, _) in events {
        let text = match event {
            Event::Text(t) | Event::InlineHtml(t) => t.as_ref(),
            Event::Code(t) => {
                let mut code = style.clone();
                code.code = true;
                code.to_richtext(ui, t).append_to(
                    &mut job,
                    ui.style(),
            ...
            | Event::End(TagEnd::TableCell | TagEnd::TableRow | TagEnd::TableHead) => continue,
            _ => return None,
        };
        style.to_richtext(ui, text).append_to(
            &mut job,
            ui.style(),
            egui::FontSelection::Default,
            egui::Align::Min,
...
impl CommonMarkViewerInternal {
    ...
    fn blockquote<'e>(
        ...
        if self.is_blockquote {
            // Match the quote's own closing event, not a nested quote's close.
            let mut collected_events = Vec::new();
            let mut depth = 1usize;
            for (_, (event, span)) in events.by_ref() {
                match &event {
                    pulldown_cmark::Event::Start(pulldown_cmark::Tag::BlockQuote(_)) => depth += 1,
                    pulldown_cmark::Event::End(pulldown_cmark::TagEnd::BlockQuote(_)) => depth -= 1,
                    _ => {}
                }
                collected_events.push((event, span));
                if depth == 0 {
                    break;
                }
            }
            ...
            if let Some(alert) = parse_alerts(&options.alerts, &mut collected_events) {
                egui_commonmark_backend::alert_ui(alert, ui, |ui| {
                    let mut iter = collected_events.into_iter().enumerate().peekable();
                    while let Some((_, (event, span))) = iter.next() {
                        self.process_event(ui, &mut iter, event, span, cache, options, max_width);
                    }
                })
            } else {
    ...
    fn table<'e>(
        ...
        if self.is_table {
            ...
            // Neo: keep the table in its own horizontal viewport. Long cells wrap;
            // many columns scroll instead of expanding the surrounding message.
            let Table { header, rows } = parse_table(events);
            let columns = header.len().max(1);
            let font_size = ui.text_style_height(&TextStyle::Body);
            let gap = font_size * 0.8;
            let padding = font_size * 0.45;
            let viewport_width = max_width.min(ui.available_width()).max(1.0);
            let cell_width = ((viewport_width - padding * 2.0 - gap * (columns - 1) as f32)
                / columns as f32)
                .max(font_size * 6.0);
            let table_width = cell_width * columns as f32 + gap * (columns - 1) as f32;
            let alignments = self.table_alignments.clone();
            egui::ScrollArea::horizontal()
                .id_salt(id)
                .max_width(viewport_width)
                .scroll_bar_visibility(if table_width + padding * 2.0 > viewport_width + 1.0 {
                    ...
                    egui::Frame::group(ui.style())
                        ...
                        .show(ui, |ui| {
                            egui::Grid::new(id.with("grid"))
                                .striped(true)
                                .spacing(egui::vec2(gap, font_size * 0.55))
                                .min_col_width(cell_width)
                                .max_col_width(cell_width)
                                .show(ui, |ui| {
                                    ui.set_width(table_width);
                                    for (row_index, row) in
                                        std::iter::once(header).chain(rows).enumerate()
                                    {
                                        if row.is_empty() {
                                            continue;
                                        }
                                        for (column_index, col) in row.into_iter().enumerate() {
                                            let align = match alignments.get(column_index) {
                                            ...
                                            // Grid centers the REQUESTED height before the child grows.
                                            // A zero-height request starts text at the row's midpoint.
                                            // Measure and reuse the exact galley instead of guessing a
                                            // font height (wrapped text and inline code differ).
                                            let galley = table_text_job(ui, &col, row_index == 0)
                                                .map(|mut job| {
                                                    job.wrap.max_width = cell_width;
                                                    job.halign = align;
                                                    job.justify = false;
                                                    ui.fonts_mut(|fonts| fonts.layout_job(job))
                                                });
                                            let height_id =
                                                id.with(("cell-height", row_index, column_index));
                                            let height = galley.as_ref().map_or_else(
                                                || {
                                                    ui.data(|data| data.get_temp::<f32>(height_id))
                                                        .unwrap_or(font_size)
                                                },
                                                |galley| galley.size().y,
                                            );
                                            let complex = galley.is_none();
                                            let cell = ui.allocate_ui_with_layout(
                                                egui::vec2(cell_width, height),
                                                egui::Layout::top_down(align),
                                                |ui| {
                                                    ui.set_width(cell_width);
                                                    // The same galley is both measured and drawn. Label
                                                    // keeps selection/accessibility without justifying text.
                                            ...
                                            if complex && !ui.is_sizing_pass() {
                                                // Arbitrary math/link callbacks cannot safely be run twice
                                                // just to measure. Remember their actual height, then let
                                                // egui settle the layout in a new pass with matching hitboxes.
                                                let measured = cell.response.rect.height();
                                                ui.data_mut(|data| {
                                                    data.insert_temp(height_id, measured)
                                                });
                                                if (measured - height).abs() > 0.5 {
                                                    ui.ctx().request_discard(
                                                        "table cell height changed",
                                                    );
        ...
        match tag {
            ...
            pulldown_cmark::TagEnd::Image => {
                if let Some(image) = self.image.take() {
                    // Model-authored Markdown must not silently read local files or
                    // make tracking requests. Keep alt text and an explicit web link.
                    let alt = image.alt_text.iter().map(|s| s.text()).collect::<String>();
                    let label = format!("[图片：{}]", if alt.is_empty() { "无描述" } else { &alt });
                    if safe_link(&image.uri) && !image.uri.starts_with('#') {
                        ui.hyperlink_to(label, &image.uri)
                            .on_hover_text("点击在浏览器打开；不会自动加载外部图片");
                    } else {
                        ui.label(label)
                            .on_hover_text("未自动读取本地或嵌入图片，请通过附件或图像工具查看");
                    }
                }
            }
